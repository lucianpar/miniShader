...
public:
  ...
  const int VIDEO_HEIGHT = 1080;  // 1080p height
  const float FPS = 39.0f;        // 39fps
  const float DURATION = 120.0f;  // 2 minutes
  const int BITRATE = 6000;       // 6Mbps for 1080p quality
  ...
  void onInit() override {
    ...
    std::string fragSearchPath = al::File::currentPath() + fragFolder;
    std::cout << "Current path: " << al::File::currentPath() << std::endl;
    std::cout << "Search path: " << searchPath << std::endl;
    std::cout << "Frag search path: " << fragSearchPath << std::endl;
    searchPaths.addSearchPath(searchPath);
    searchPaths.addSearchPath(fragSearchPath);
