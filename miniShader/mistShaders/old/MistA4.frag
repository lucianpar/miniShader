...
vec4 shaderSmoke3(vec2 uv, float t) {
    ...
    // Use fbm for base structure, with smaller scale for distinct dots
    float mist = fbm(vec3(uv * 10.0, t * 0.8));  // Higher frequency for smaller structures

    // Sharpen and isolate the cloud dots
    float cloudDots = smoothstep(0.6, 0.8, mist);  // Isolate brighter regions
    cloudDots = pow(cloudDots, 6.0);  // Sharpen the dots

    // Add grain for texture, but keep it subtle
    float grain = grainNoise(uv * 50.0, t);  // Moderate frequency for grain
...
void main() {
    ...
    // Use shaderSmoke3 for a collection of little cloud dots
