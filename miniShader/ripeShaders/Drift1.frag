...
// Increase the scale and amplitude of the membranes to engulf more of the screen
float wave(vec2 p)
{
    p *= 1.5; // scale up
    float v = sin(p.x + sin(p.y * 2.0) + sin(p.y * 0.43));
    v += 0.7 * sin(p.y * 1.5 + u_time * 0.2); // increased amplitude
    v += 0.5 * cos(p.x * 1.7 - u_time * 0.15); // increased amplitude
    return v * mod(v, p.x + 1.2) / (p.y + 1.1);
}
...
float wave2(vec2 p)
{
    ...
    return v * mod(v, p.y + 1.3) / (p.x + 1.1);
}
...
vec3 Oceanic_Membrane(vec2 pos)
{
    ...
    float intersection = smoothstep(0.05, 0.0, abs(v1 - v2));
