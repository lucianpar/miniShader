...
vec2 domainWarp(in vec2 p, in float speed, in float amp){
    ...
    float sc = 0.8 + (0.7*(u_time/150.0)); // main warping resolution
    vec2 q = p;
    for(int i=0;i<3;i++){
...
void main() {
    ...
    float zoom = 1.0;
    float fadeToBlack = 1.8; //main darknes shere
    float density1 = 4.0; // density for swarm1
    ...
    float thickness1 = mix(0.5, 3.0, t / 120.0); // start at 0.5
    ...
    if (u_time > transitionTime || u_time <= transitionTime) {
        if (u_time > transitionTime) {
            ...
            float growthFactor = mix(0.8, 1.3, (t-transitionTime) / 60.0); // start super tiny, slow growth over entire track
