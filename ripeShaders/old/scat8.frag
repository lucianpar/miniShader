...
vec3 renderBugSwarms(vec2 uv, float thickness1, float thickness2) {
    ...
    // Swarm 1: default purple swarm with animated thickness
    totalColor += renderBugSwarm(uv, vec3(1.0), 0.08, 1.0, thickness1);
    
    // Swarm 2: rotated, less dense, bright green, thicker with animated thickness
...
void main() {
    ...
    swarm1Brightness = sin(u_time); // brightness oscillates between -1 and 1
    vec2 uv = vPos.xy * 2.0;

    // Animate thickness parameters in main for easy control
    float thickness1 = 1.0 + u_time * 0.02; // gradual increase
    float thickness2 = 1.5 + u_time * 0.03; // faster increase

    vec3 finalColor = vec3(0.0);

    // Conditionally render swarms to save GPU: run full swarms only when brightness > 0
    if (swarm1Brightness > 0.0) {
        finalColor = renderBugSwarms(uv, thickness1, thickness2) * swarm1Brightness;
    } else {
        // Run the unique thin green-white swarm when brightness <= 0, modulated by cosine
        swarm2Brightness = -1.0 * sin(u_time);
        finalColor = renderBugSwarm(uv, vec3(0.3, 1.0, 0.1) + 0.1, 0.4, 0.8, 2.9) * swarm2Brightness;
        // Mask to a ball in the center with softer edges
        float dist = length(uv);
        float mask = 1.0 - smoothstep(3.0, 8.0, dist); // softer transition from 4.0 to 6.0
        finalColor *= mask;
    }
