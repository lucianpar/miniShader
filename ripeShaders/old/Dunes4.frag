...
vec3 renderSunBase(vec2 uv, float r, float body) {
    vec3 plasma = Solar_Plasma(uv * 3.0);
    vec3 sunColor = plasma * body;

    // Dark purple inner tendrils
...
vec3 renderSunEffects(vec2 uv, float r, vec3 sunColor) {
    // Glow halo
...
void main() {
    ...
    // === Modular rendering ===
    vec3 sunBase = renderSunBase(uv, r, body);
    vec3 sunWithSand = renderSand(uv, r, body, sunBase);
    vec3 finalSun = renderSunEffects(uv, r, sunWithSand);
