...
void main() {
    float t = u_time;
    if (t > TRACK_LENGTH) t = TRACK_LENGTH; // clamp

    // Timeline variables
    float swarm1Brightness = 0.0;
    float swarm2Brightness = 0.0;
    float speedMult = 1.0; // always normal, no speed changes
    vec3 colorMult1 = vec3(1.0); // additional color for swarm1 later
    float zoom = 1.0;
    float fadeToBlack = 1.0;
    float density1 = 1.0; // density for swarm1

    // SWARM 1 phases with 2s fade in, 1s fade out
    if (t >= 3.5 && t < 14.0) {
        float fadeIn = smoothstep(3.5, 5.5, t);
        float fadeOut = 1.0 - smoothstep(12.0, 14.0, t);
        swarm1Brightness = fadeIn * fadeOut;
    } else if (t >= 15.0 && t < 31.0) {
        float fadeIn = smoothstep(14.0, 16.0, t);
        float fadeOut = 1.0 - smoothstep(30.0, 31.0, t);
        swarm1Brightness = fadeIn * fadeOut;
    } else if (t >= 33.0 && t < 42.0) {
        float fadeIn = smoothstep(33.0, 35.0, t);
        float fadeOut = 1.0 - smoothstep(41.0, 42.0, t);
        swarm1Brightness = fadeIn * fadeOut;
    } else if (t >= 45.0 && t < 51.0) {
        float fadeIn = smoothstep(45.0, 47.0, t);
        float fadeOut = 1.0 - smoothstep(50.0, 51.0, t);
        swarm1Brightness = fadeIn * fadeOut;
    } else if (t >= 76.5 && t < 204.0) {
        swarm1Brightness = 1.0; // always on, fade out by density and thickness
        density1 = mix(1.0, 0.0, (t - 76.5) / (204.0 - 76.5)); // decrease density
        colorMult1 = vec3(1.5, 1.2, 1.0); // additional color
        speedMult = 1.5; // faster regular speed
    }

    // SWARM 2 phases with 2s fade in, 1s fade out
    if (t >= 52.5 && t < 59.0) {
        float fadeIn = smoothstep(52.5, 54.5, t);
        float fadeOut = 1.0 - smoothstep(58.0, 59.0, t);
        swarm2Brightness = fadeIn * fadeOut;
    } else if (t >= 62.5 && t < 71.0) {
        float fadeIn = smoothstep(62.5, 64.5, t);
        float fadeOut = 1.0 - smoothstep(70.0, 71.0, t);
        swarm2Brightness = fadeIn * fadeOut;
    } else if (t >= 176.0) {
        float fadeIn = smoothstep(176.0, 178.0, t);
        float fadeOut = 1.0 - smoothstep(186.0, 187.0, t); // adjust for fade to black
        swarm2Brightness = fadeIn * fadeOut;
        zoom = mix(1.0, 2.0, (t - 176.0) / 11.0);
        if (t >= 187.0) fadeToBlack = 1.0 - smoothstep(187.0, 204.0, t); // smoother fade to black
    }
    ...
    float thickness1 = mix(0.5, 3.0, t / 100.0); // start at 0.5
    if (t > 100.0) thickness1 *= (204.0 - t) / 104.0;
    float thickness2 = 1.5 + t * 0.03; // faster increase
