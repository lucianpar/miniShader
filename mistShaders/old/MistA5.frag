...
float fbm1(vec3 seed) {
    float result = 0.0;
    float a = 0.5;
    for (int i = 0; i < 30; ++i) {
        seed.z += result * 0.5;
        result += abs(gyroid1(seed / a)) * a;
        a *= 0.6;
    }
    return result;
}
...
vec4 shaderSmoke1(vec2 uv, float t) {
    ...
    // Use fbm1 for base structure, with smaller scale for distinct dots
    float mist = fbm1(vec3(uv * 10.0, t * 0.1));  // Higher frequency for smaller structures
    ...
    float grain = grainNoise(uv * 2.0, t);  // Moderate frequency for grain
...
void main() {
    ...
    // Use shaderSmoke1 for a collection of little cloud dots
    vec4 finalColor = shaderSmoke1(uv, t);
